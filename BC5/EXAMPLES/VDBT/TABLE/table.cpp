...
//
// table.cpp - VDBT PITTtabe example
/*******************************************************************************
This example demonstrates how hook up to a database table and then navigate
through it using the VDBT PITtable object and the COM classes.  The user can
...
table.cpp			Source file for application
table.rc				Resource file for application
table.ico			Icon file for application
table16/32.def		module definition file for 16bit and 32bit target respectively
makefile				makefile for command-line tools build
table.ide			BCW 5.0 project file to build both 32bit and 16bit	targets.
*******************************************************************************/
...
BOOL RegisterMainClass( HINSTANCE hInstance )
{
	...
	wndClass.cbWndExtra = sizeof(PITTable) + sizeof(PPITField) + sizeof(PITDataSource);
...
extern "C" LRESULT CALLBACK _export MainWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	switch (msg)
	{
		case WM_CREATE:
...
void MainCreateMsg(HWND hwnd, LPSTR lpszCmdLine)
{
	...
	if (table)
	{
		...
		SetWindowLong( hwnd, 0, (LONG) table );
...
void MainPaintMsg(HWND hwnd)
{
	...
	PITTable table = (PITTable) GetWindowLong( hwnd, 0 );
	if (table)
	{
		int count = table->get_FieldCount();
		if (count)
...
void MainDestroyMsg( HWND hwnd )
{
	// Release the table, the fields, and the datasource.
