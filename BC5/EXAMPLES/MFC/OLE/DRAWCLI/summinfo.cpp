...
// the following header redefines the DEFINE_GUID macro to actually allocate data
#include <initguid.h>
#ifndef INITGUID
#define INITGUID
#endif
...
BOOL CSummInfo::AddCountToEditTime()
{
	FILETIME now;
	CoFileTimeNow(&now);
	double currTime = *(double*)&now;
	double thisSession = currTime - startEdit;
	double lastTotal = *(double*)m_pSection->Get(PID_EDITTIME);
	double newTotal = lastTotal + thisSession;
